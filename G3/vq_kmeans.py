...
def main(args):
    ...
    print("="*60)
    print(f"Ks testados (Pixel-VQ) = {len(Ks_px)} valores de {min(Ks_px)} até {max(Ks_px)} (K_max teórico: {Kmax_px})")
    print(f"Ks testados (VQ 2×2)   = {len(Ks_pt)} valores de {min(Ks_pt)} até {max(Ks_pt)} (K_max teórico: {Kmax_pt})")
